function sum_test_data_generator
...
xzero   = [2,3,4].';
params  = [0.1, 0.2, 0.3].';
[~,xsim] = ode15s(@(t,x) sum_test_fun(t,x,params),tspan,xzero,my_opts); % change fun to asses diferent models
data = xsim(:,3);
dt = 0.1*abs(mean(data));
noise = dt.*normrnd(0,1,size(data));
data_noise = data + noise;

save('sum_test_data','tspan','xzero','params','data_noise');